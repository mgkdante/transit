...
def _seed(connection) -> None:
    # FULL parent chain: core.providers requires provider_key (wave-3 schema).
    connection.execute(
        text(
            """
            INSERT INTO core.providers (provider_id, display_name, timezone, provider_key)
            VALUES (:p, 'STM snapshot-state regression', 'America/Toronto', :p)
            """
        ),
        {"p": PROVIDER},
    )
...
def test_state_upsert_is_idempotent(conn) -> None:
    ...
    _record_publish_state(
        conn, provider_id=PROVIDER, tier="static", generated_utc=T1,
        written=9300, skipped=0, total=9300,
    )
    ...
    _record_publish_state(
        conn, provider_id=PROVIDER, tier="static", generated_utc=T2,
        written=20, skipped=9280, total=9300,
    )
...
def test_state_fk_rejects_unknown_provider(conn) -> None:
    ...
    with pytest.raises(IntegrityError):
        _record_publish_state(
            conn, provider_id="ghost_provider", tier="static",
            generated_utc=T1, written=0, skipped=0, total=0,
        )
...
def test_tier_check_constraint(conn) -> None:
    ...
    with pytest.raises(IntegrityError):
        _record_publish_state(
            conn, provider_id=PROVIDER, tier="bogus",
            generated_utc=T1, written=0, skipped=0, total=0,
        )
...
def test_build_manifest_reads_tier_state(conn) -> None:
    ...
    _record_publish_state(
        conn, provider_id=PROVIDER, tier="static", generated_utc=T1,
        written=10, skipped=0, total=10,
    )
    _record_publish_state(
        conn, provider_id=PROVIDER, tier="historic", generated_utc=T2,
        written=5, skipped=0, total=5,
    )
