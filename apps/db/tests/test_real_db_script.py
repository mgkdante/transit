...
if tool == "docker":
    ...
    if arguments[:2] == ["volume", "inspect"]:
        raise SystemExit(0 if scenario == "residual-volume" else 1)
    raise SystemExit(97)

if arguments == ["run", "alembic", "upgrade", "head"]:
    if scenario == "term-during-alembic":
        time.sleep(60)
    raise SystemExit(41 if scenario == "alembic-failure" else 0)
...
def _install_fakes(fake_bin: Path, *, include_docker: bool = True, include_uv: bool = True) -> None:
    fake_bin.mkdir()
    for name in ("bash", "dirname", "od", "tr"):
...
def _run_scenario(
    ...
    *,
    ...
    _install_fakes(fake_bin, include_docker=include_docker, include_uv=include_uv)
    ...
    return result, _events(log_path) if log_path.exists() else []
...
def test_script_maps_term_and_cleans_up(tmp_path: Path) -> None:
    fake_bin = tmp_path / "bin"
    log_path = tmp_path / "commands.jsonl"
    _install_fakes(fake_bin)
    process = subprocess.Popen(
        ["bash", str(SCRIPT_PATH)],
        cwd=REPO_ROOT,
        env=_script_environment(fake_bin, log_path, "term-during-alembic"),
        text=True,
    ...
    while time.monotonic() < deadline:
        if log_path.exists() and '"tool": "uv"' in log_path.read_text(encoding="utf-8"):
            break
        time.sleep(0.02)
    else:
        process.kill()
...
def test_script_cleanup_defects_override_success(
    ...
    assert expected_message in result.stderr
    assert [_operation(event) for event in events][-2:] == ["down", "volume-inspect"]
