"""Real-DB regression for the stop_delay_spine BUILDER (DB-PR-3, the MANDATORY facts->builder->table gate).
...
pytestmark = pytest.mark.skipif(
    not DB_URL, reason="TRANSIT_TEST_DATABASE_URL not set - stop spine builder real-DB gate skipped"
)
...
def _provider_and_endpoint(connection) -> None:  # noqa: ANN001
    connection.execute(
        text(
            ...
            "VALUES (:p, 'STM stop builder', 'America/Toronto', :p)"
        ),
        {"p": PROVIDER},
    )
    connection.execute(
...
def _seed_rows(connection, local_date, rows, base=997100) -> None:  # noqa: ANN001
    ...
    for i, (stop_id, route_id, delay) in enumerate(rows):
        ...
        connection.execute(
            ...
            {"p": PROVIDER, "s": sid, "dk": date_key, "sld": local_date, "ts": captured_at,
             "entity": f"e{sid}", "trip": f"t{sid}", "route": route_id, "delay": delay,
             "stop": stop_id},
        )
...
def test_builder_boundary_oracle_counts_clamp_and_severe() -> None:
    ...
    Kills: a dropped ghost-clamp (3601 leaks -> obs=7), a >= vs > severe flip (300 leaks -> severe=3),
    a 10x sum bug, and a NULL-delay leak into the count."""
    cld = _recent_weekday()
...
def test_builder_null_route_lands_under_unrouted_sentinel() -> None:
    """A NULL route_id stop observation is attributed to the '__unrouted__' sentinel route, not
    dropped — the per-stop totals depend on it (a real per-route read never matches the sentinel)."""
    cld = _recent_weekday()
...
def test_builder_idempotent_rebuild() -> None:
    """Building the same closed day twice yields identical rows (ON CONFLICT upsert) + one watermark."""
    cld = _recent_weekday()
    ...
    with _conn() as conn:
        ...
        _build(conn)  # resumable build skips the already-watermarked day -> still one row, unchanged
