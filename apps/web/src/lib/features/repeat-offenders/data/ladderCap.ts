// ladderCap — the S14 repeat-offenders worst-N ladder cap helpers (data-only; no
// runes, no locale). A verbatim reuse of the S12 hotspots ladderCap shape.
//
// The worst-N cap is a codec-owned FilterState axis (?n — see $lib/filters): one of
// the fixed truthful rungs WORST_N_LADDER (5/10/20/30/50) or the uncapped 'all',
...
//
// TRUTH DISCIPLINE: the cap is a DISPLAY axis only. Truncating to N never rescales
// the ranks — the ladder's absolute domain [0,100] is fixed and the DB Wilson-LB
// order is preserved. A smaller N shows fewer rows of the SAME bars, never longer ones.

import { WORST_N_LADDER, type WorstN } from '$lib/filters';
import type { GrainSegment } from '$lib/components/surface';

/** The surface default cap when ?n is absent. */
