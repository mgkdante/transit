...
def test_parse_static_key_extracts_date_and_timestamp() -> None:
    key = "stm/static_schedule/ingested_at_utc=2026-04-30/20260430T120000000000Z__abc123def456__gtfs.zip"
...
def test_parse_realtime_key_extracts_date_and_timestamp() -> None:
    key = "stm/trip_updates/captured_at_utc=2026-05-01/20260501T003015123456Z__feedabc12345__trip_updates.pb"
...
def test_parse_unknown_key_returns_none() -> None:
    ...
    assert (
        parse_bronze_key(
            "stm/alerts/captured_at_utc=2026-04-30/20260430T120000000000Z__old__alerts.pb"
        )
        is None
    )
...
def test_cleanup_plan_deletes_only_pre_may_known_keys() -> None:
    storage = FakeListDeleteStorage(
        [
            ...
            "stm/trip_updates/captured_at_utc=2026-05-01/20260501T000000000000Z__keep__trip_updates.pb",
...
def test_cleanup_plan_keeps_may_and_later_known_keys() -> None:
    storage = FakeListDeleteStorage(
        [
            "stm/static_schedule/ingested_at_utc=2026-04-30/20260430T120000000000Z__old__gtfs.zip",
            "stm/static_schedule/ingested_at_utc=2026-05-01/20260501T000000000000Z__may1__gtfs.zip",
            "stm/vehicle_positions/captured_at_utc=2026-05-02/20260502T010000000000Z__may2__vehicle_positions.pb",
        ]
    )
...
def test_cleanup_plan_reports_malformed_known_keys_as_unknown() -> None:
    storage = FakeListDeleteStorage(
        [
            "stm/static_schedule/ingested_at_utc=not-a-date/20260430T120000000000Z__old__gtfs.zip",
            "stm/vehicle_positions/captured_at_utc=2026-04-30/not-a-timestamp__old__vehicle_positions.pb",
        ]
    )
...
def test_execute_cleanup_plan_collects_delete_failures_and_keeps_going() -> None:
    failing_key = (
        ...
        "20260430T120000000000Z__fail__trip_updates.pb"
    )
    successful_key = (
        "stm/vehicle_positions/captured_at_utc=2026-04-30/"
        "20260430T120500000000Z__ok__vehicle_positions.pb"
    )
    storage = FakeListDeleteStorage(
