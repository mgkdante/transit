...
const HEADER = `/* GENERATED FROM tools/tokens/tokens.json — DO NOT EDIT */
...
function pairedBlock(items: FlatToken[], indent: string): string {
	...
	const lines = PAIRED_DEFS.filter((p) => !explicit.has(p.name)).map((p) => `${indent}${p.line}`);
...
export function generateTokensCss(tree: TokenTree): string {
	...
	const rootExtra =
		'  /* shadcn alias — components reference --radius */\n  --radius: var(--radius-md);';

	const darkBlock = emitBlock(
		'[data-theme="dark"], .theme-dark',
		'dark',
		darkItems,
		pairedBlock(darkItems, '  '),
	);
	const lightBlock = emitBlock(
		'[data-theme="light"], .theme-light',
		'light',
		lightItems,
		pairedBlock(lightItems, '  '),
	);
